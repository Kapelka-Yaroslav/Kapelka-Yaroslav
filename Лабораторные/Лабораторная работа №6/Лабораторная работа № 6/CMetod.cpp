...
{
	std::cout << "Title " << "Number_of_cities " << "Population " << "Area " << "Place_of_birth_citizen " << "Birthday_citizen " << "\n";
...
{
	std::stringstream ss;
	ss << "_" << countries[index].getTitle() << "_ " << countries[index].getNumber_of_cities() << " " << countries[index].getPopulation() << " " << countries[index].getArea() << " " << countries[index].getPlace_of_birth_citizen() << " " << countries[index].getBirthday_citizen();
	return ss.str();
}
void CMetod::find_to_str_by_file(const std::string str)
{
	if (check_str(str))
	{
		...
		s << temp;
		CCountry secondcountry(Title, Number_of_cities, Population, Area, Place_of_birth_citizen, Birthday_citizen);
...
void Inheritor_CMetod::add_el(const Inheritor_ÑÑountry& Inheritor_Ñountry)
{
	if (next_i == 0)
	{
		countries = new Inheritor_ÑÑountry[1];
		countries[next_i] = Inheritor_Ñountry;
		next_i++;
	}
	else
	{
		copy = new Inheritor_ÑÑountry[next_i + 1];
		for (int i = 0; i < next_i; i++)
		{
			copy[i] = countries[i];
		}
		delete[] countries;
		countries = new Inheritor_ÑÑountry[next_i + 1];
		for (int i = 0; i < next_i; i++)
		{
			countries[i] = copy[i];
		}
...
void Inheritor_CMetod::remove_el(const int& index)
{
	if (next_i == 1)
	{
		delete[] countries;
...
void Inheritor_CMetod::get_to_Screen(const int& index) const
{
	std::cout << "Title " << "Number_of_cities " << "Population " << "Area " << "Place_of_birth_citizen " << "Birthday_citizen " << "Monarchy " << "\n";
	std::cout << get_str_by_file(index) << "\n";
}
Inheritor_ÑÑountry Inheritor_CMetod::find_to_index(const int& index) const
{
	for (int i = 0; i < next_i; i++)
	{
		if (countries[i].getUnical_index() == index)
...
void Inheritor_CMetod::print_all() const
{
	for (int i = 0; i < next_i; i++)
	{
		get_to_Screen(i);
...
void Inheritor_CMetod::find_to_population_density() const
{
	float min = countries[0].getPopulation_density();
	for (int i = 0; i < next_i; i++)
	{
...
std::string Inheritor_CMetod::get_str_by_file(const int& index) const
{
	std::stringstream ss;
	ss << "_" << countries[index].getTitle() << "_ " << countries[index].getNumber_of_cities() << " " << countries[index].getPopulation() << " " << countries[index].getArea() << " " << countries[index].getPlace_of_birth_citizen() << " " << countries[index].getBirthday_citizen() << " " << countries[index].getMonarchy();
	return ss.str();
}
void Inheritor_CMetod::find_to_str_by_file(const std::string str)
{
	if (check_str(str))
	{
		std::regex reg("_.+_");
		...
		s << temp;
		Inheritor_ÑÑountry secondcountry(Title, Number_of_cities, Population, Area, Place_of_birth_citizen, Birthday_citizen, Monarchy);
		add_el(secondcountry);
	}
}
void Inheritor_CMetod::write_to_file(const std::string name)
{
...
void Inheritor_CMetod::read_from_file(const std::string name)
{
	del_all();
...
bool Inheritor_CMetod::check_str(const std::string& str) const
{
	std::regex reg("[A-Za-zA-ßà-ÿ0-9\!,\?\"\/:;\']*");
	if (!(std::regex_search(str, reg)))
	{
...
void Inheritor_CMetod::print_all_with_2_or_more_words() const
{
	for (int i = 0; i < next_i; i++)
	{
		std::string str;
...
void Inheritor_CMetod::Inheritor_sort(bool (*comp)(Inheritor_ÑÑountry&, Inheritor_ÑÑountry&))
{
	bool pr = false;
	Inheritor_ÑÑountry temp;
	do
	{
		pr = false;
		for (int i = 0; i < next_i - 1; i++)
...
bool sortTitle(Inheritor_ÑÑountry& a, Inheritor_ÑÑountry& b)
{
	return (a.getTitle() > b.getTitle());
}
bool sortNumber_of_cities(Inheritor_ÑÑountry& a, Inheritor_ÑÑountry& b)
{
	return(a.getNumber_of_cities() < b.getNumber_of_cities());
}
bool sortPopulation(Inheritor_ÑÑountry& a, Inheritor_ÑÑountry& b)
{
	return(a.getPopulation() < b.getPopulation());
}
bool sortArea(Inheritor_ÑÑountry& a, Inheritor_ÑÑountry& b)
{
	return (a.getArea() < b.getArea());
}
bool sortCitizen(Inheritor_ÑÑountry& a, Inheritor_ÑÑountry& b)
{
	return((a.getPlace_of_birth_citizen() > b.getPlace_of_birth_citizen()) && (a.getBirthday_citizen() < b.getBirthday_citizen()));
}
bool sortMonarchy(Inheritor_ÑÑountry& a, Inheritor_ÑÑountry& b)
{
	return (a.getMonarchy() < b.getMonarchy());
}